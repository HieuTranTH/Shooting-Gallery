...
static uint16_t mo3_green_buff[] = { 0x0000,   			  //
		0xEEEE, 0xEEEE, 0x8888, 0x8888, 0x8888, 0x8888, //
		0xEEEE, 0xEEEE, 0x8888, 0x8888, 0x8888, 0x8888, //
		0xEEEE, 0xEEEE, 0x8888, 0x8888, 0x8888, 0x8888, //
		0xEEEE, 0xEEEE, 0x8888, 0x8888, 0x8888, 0x8888, //
		0xEEEE, 0xEEEE, 0x8888, 0x8888, 0x8888, 0x8888, //
...
static uint16_t mo3_red_buff[] = { 0x0000,			 //
		0x8888, 0x8888, 0xEEEE, 0xEEEE, 0x8888, 0x8888,  //
		0x8888, 0x8888, 0xEEEE, 0xEEEE, 0x8888, 0x8888,  //
		0x8888, 0x8888, 0xEEEE, 0xEEEE, 0x8888, 0x8888,  //
		0x8888, 0x8888, 0xEEEE, 0xEEEE, 0x8888, 0x8888,  //
		0x8888, 0x8888, 0xEEEE, 0xEEEE, 0x8888, 0x8888,  //
...
static volatile xSemaphoreHandle LEDStripMutex = xSemaphoreCreateMutex();
...
void greenLED(unsigned int target, bool state) {
	...
	if (xSemaphoreTake(LEDStripMutex, portMAX_DELAY) == pdTRUE) {
		...
		if (state) {
			WriteSpi0Mssg(mo3_green_buff,
					sizeof(mo3_green_buff) / sizeof(mo3_green_buff[0]), 3,
					mi3_off_buff);
		} else {
			WriteSpi0Mssg(mo3_off_buff,
					sizeof(mo3_off_buff) / sizeof(mo3_off_buff[0]), 3,
					mi3_off_buff);
		}
...
void redLED(unsigned int target, bool state) {
	...
	if (xSemaphoreTake(LEDStripMutex, portMAX_DELAY) == pdTRUE) {
		...
		/*
		 ...
		 3, mi3_off_buff);*/
		if (state) {
			WriteSpi0Mssg(mo3_red_buff,
					sizeof(mo3_red_buff) / sizeof(mo3_red_buff[0]), 3,
					mi3_off_buff);
		} else {
			WriteSpi0Mssg(mo3_off_buff,
					sizeof(mo3_off_buff) / sizeof(mo3_off_buff[0]), 3,
...
void vTargetSendToQueueTask(void *pvParameters) {
	...
	while (1) {
		if (xSemaphoreTake(clearPININT0, portMAX_DELAY) == pdTRUE) {
			...
			if ((target != 0) && ((newtick - oldtick) >= 500)) {
		...
		if (xSemaphoreTake(soundStartSem, portMAX_DELAY) == pdTRUE) {
			if (soundToggleV) {
				if (xSemaphoreTake(SoundMutex, portMAX_DELAY) == pdTRUE) {
					...
					LPC_SCT0->CTRL_L &= ~(1 << 2); // unhalt by clearing bit 2 of CTRL register
					LPC_SCT0->MATCHREL[0].U = (SystemCoreClock / (698.46 * 2))
							- 1;
					vTaskDelay(250);
					LPC_SCT0->MATCHREL[0].U = (SystemCoreClock / (783.99 * 2))
							- 1;
					vTaskDelay(250);
					LPC_SCT0->MATCHREL[0].U = (SystemCoreClock / (880.00 * 2))
							- 1;
					vTaskDelay(250);
		...
		if (xSemaphoreTake(soundMissSem, portMAX_DELAY) == pdTRUE) {
			if (soundToggleV) {
				if (xSemaphoreTake(SoundMutex, portMAX_DELAY) == pdTRUE) {
					...
					LPC_SCT0->CTRL_L &= ~(1 << 2); // unhalt by clearing bit 2 of CTRL register
					LPC_SCT0->MATCHREL[0].U = (SystemCoreClock / (87.31 * 2))
							- 1;
					vTaskDelay(1000);
					LPC_SCT0->CTRL_L |= (1 << 2); // halt by setting bit 2 of CTRL register
...
int main(void) {
	...
	/*************************************************************************************/
	if (button_cannon->read()) {
		while (button_cannon->read()) {
		}
#if 1
